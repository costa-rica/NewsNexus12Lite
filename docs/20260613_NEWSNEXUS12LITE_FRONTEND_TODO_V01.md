...
# NewsNexus12Lite — Frontend TODO V01
...
- [ ] Create `portal/src/store/promptsSlice.ts`:
  - State: `defaults: PromptConfiguration | null`, `session: PromptConfiguration | null`, `dirtyFlags: { approver: boolean, stateAssigner: boolean }`, `validationErrors: Record<string, string>`.
  - Actions: `setDefaults`, `setSessionPrompts`, `updateApproverPrompts`, `updateStateAssignerPrompt`, `markClean`, `resetPromptGroup`.
  - `resetPromptGroup(group: "approver" | "stateAssigner")` — copies `defaults[group]` back into `session[group]` and sets the dirty flag to false.
  - Thunk: `loadPrompts` — calls `GET /api/prompts`; populates both `defaults` and `session`.
  - Thunk: `applyPrompts(scope)` — calls `PUT /api/prompts`; on success calls `markClean(scope)`.
...
- [ ] Select "No, continue" and confirm the modal dismisses. Refresh — confirm the modal appears again (no persisted `firstLaunchAnswered: true` without a prior "Yes" or "No" choice in sessionStorage).
...
- [ ] Add a run-ID guard in `articlesSlice.updateArticleFromSnapshot`: ignore snapshots where the embedded `runId` does not match `orchestration.runId`. This prevents stale responses from a cancelled run repopulating the UI.
- [ ] Write tests for the polling logic:
  ...
  - Snapshot with a stale `runId` is discarded.
...
- [ ] Create `portal/src/components/ApproverPromptsPage.tsx` (the page's primary component):
  - **Session status badge**: "Default" (green) or "Modified" (amber) based on `prompts.dirtyFlags.approver`.
  - **Gateway Prompt editor**:
    - Label: "Gateway Prompt".
    - A multiline `<textarea>` pre-filled with `prompts.session.approver.gatewayPrompt`.
    - On change: updates local component state (not Redux until the user applies).
  ...
  - **Action buttons**:
    - `[Apply to Session]`: calls `PUT /api/prompts { scope: "approver", prompts: { gateway, hazards } }`. On success, dispatches `promptsSlice.updateApproverPrompts` with the new values and `promptsSlice.markClean("approver")`.
...
- [ ] Write tests for `<ApproverPromptsPage>`:
  ...
  - Session status badge shows "Modified" after an edit.
...
- [ ] Start the dev server. Navigate to `/prompts/approver` via the sidebar. Confirm:
  ...
  - Editing and clicking "Apply to Session" changes the session status badge to "Modified".
...
- [ ] Create `portal/src/components/StateAssignerPromptsPage.tsx`:
  - **Session status badge**: "Default" or "Modified" based on `prompts.dirtyFlags.stateAssigner`.
  - **State Assignment Prompt** (single editable field):
    ...
    - On change: updates local component state.
  ...
  - **Action buttons**:
    - `[Apply to Session]`: calls `PUT /api/prompts { scope: "stateAssigner", prompts: { assignmentPrompt } }`. On success dispatches `promptsSlice.updateStateAssignerPrompt` and `markClean("stateAssigner")`.
