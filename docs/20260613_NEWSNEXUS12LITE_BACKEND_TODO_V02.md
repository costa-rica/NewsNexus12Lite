...
# NewsNexus12Lite — Backend TODO V02
...
- [ ] Create `api/src/services/runStore.ts` with explicit session ownership:
  - Export a `Map<string, PipelineRun>` module-level singleton for lookup by run ID.
  - Add `sessionId: string` and `articleIds: string[]` to `PipelineRun` so every run has a durable owner and ordered article list.
  - Export `createRun(sessionId: string, articleIds: string[]): PipelineRun`.
  - Export `getRun(runId: string): PipelineRun | undefined`.
  - Export `getSessionRun(session: SessionObject, runId: string): PipelineRun | undefined`, which returns a run only when `run.sessionId === session.sessionId`; all status, snapshot, and cancel endpoints must use this helper.
  - Keep `session.activeRunId` as the current UI pointer only; do not rely on it as the sole ownership check because searches/resets can clear or replace it while historical run IDs still exist in `runStore`.
- [ ] Implement `POST /api/orchestration/runs`:
  - Accept body: `{ articleIds: string[], mode?: "mock" | "live" }`.
  - Reject unexpected legacy fields such as `stages` or `useSessionPrompts` with HTTP 400; the Lite pipeline always runs the fixed five-stage sequence and uses FR-086 per-stage prompt capture, never run-start prompt snapshots.
  ...
  - Create a `PipelineRun` object (see Pipeline Plan V02 §3.1) with `sessionId: session.sessionId`, `articleIds`, `status: "running"`, all `stageStatuses` set to `"pending"`, `cancellationRequested: false`.
  - Store the run in `runStore` and set `session.activeRunId = run.runId` as the current active run pointer.
...
- [ ] Implement `GET /api/orchestration/runs/:runId`:
  - Look up run via `getSessionRun(session, runId)`; return HTTP 404 if the run does not exist or `run.sessionId !== session.sessionId`.
...
- [ ] Write tests in `api/src/routes/__tests__/orchestration.test.ts`:
  ...
  - Both GET endpoints return HTTP 404 for an unknown `runId` or a `runId` owned by a different session.
  - Cross-session ownership test: create a run under one session cookie and confirm another session cookie receives HTTP 404 when requesting status/snapshot for that run.
...
- [ ] Implement `POST /api/orchestration/runs/:runId/cancel` in `orchestration.ts`:
  - Look up run via `getSessionRun(session, runId)`; return HTTP 404 if the run does not exist or belongs to a different session.
...
- [ ] Write tests in `api/src/routes/__tests__/cancel.test.ts`:
  ...
  - Cancel a run owned by another session returns HTTP 404 and does not mutate that run.
