...
# NewsNexus12Lite — Backend Plan V02
...
| Rate limiting | express-rate-limit 8 | Single global window applied to all endpoints |
...
Nick's constraint is a **global public demo API cap of 200 requests per hour and 1,000 requests per day per IP**. This cap applies to all HTTP endpoints without exception. There is no separate higher-limit tier for polling or read-only endpoints.

The rationale: the cap reflects an operator-level budget decision for the demo environment. The polling strategy (see Frontend Plan V02 and §7.2 below) is designed so that normal demo usage stays well within this global cap. A single complete demo flow — session init, search, one run, several bubble-click explanations, and a prompt edit — consumes approximately 40–55 requests, leaving substantial headroom within the hourly budget.

### 6.1 Implementation

Two `express-rate-limit` middleware instances are applied globally to all routes:

- **Hourly window**: 200 requests per 60-minute sliding window per IP.
- **Daily window**: 1,000 requests per 24-hour sliding window per IP.

Both are applied in series via `app.use(hourlyLimiter, dailyLimiter)`. A request must pass both windows to proceed. When either limit is exceeded the server responds with HTTP 429 and a `Retry-After` header showing the number of seconds until the next slot opens.

### 6.2 Budget Guidance for Polling Design

The snapshot endpoint (`GET /api/orchestration/runs/{runId}/snapshot`, §7.1) is designed as the single combined poll target during an active run, consolidating what would otherwise be two separate polling calls. At the recommended 5-second polling interval (see Frontend Plan V02), a 5-minute run costs approximately 60 snapshot requests. Combined with non-poll actions for the same flow, a single run stays within roughly 70–80 requests — comfortably under the hourly cap for one or two runs per hour.
...
| POST | `/api/orchestration/runs/:runId/cancel` | Request cancellation of an active run; idempotent on terminal-state runs |
...
`GET /api/orchestration/runs/:runId/snapshot` is the consolidated poll endpoint that replaces the two-endpoint polling pattern from V01. It returns:
...
An `ArticleSnapshot` includes: `id`, `title`, `source`, `rowStatus`, and per-stage `{ status, score?, assignedState?, confidence? }` for all five stages. This is enough for the article table and expanded row to populate progressively without any additional per-article poll.
...
- Returns `{ runId, status: "cancelled", cancelledAt: "ISO timestamp" }` with HTTP 200.
- If the run is already in a terminal state (`complete`, `failed`, `cancelled`), returns the current status unchanged (idempotent).
- If the run is not found for the current session, returns HTTP 404.

**How a run reaches `cancelled` state:**

1. The cancel endpoint sets `run.cancellationRequested = true` on the in-memory run object and returns immediately.
2. The pipeline execution loop (Pipeline Plan V02 §3) checks `run.cancellationRequested` after completing each full stage across all articles.
...
4. Any stage that is currently in progress for individual articles completes normally before the cancellation check runs; this avoids partially-written article stage state.
5. The next snapshot poll response the frontend receives will carry `status: "cancelled"`, at which point polling stops.

The `cancellationRequested` field is internal to the run object and is not exposed in API responses.
...
| Rate limit scope | Single global limit: 200/hr + 1,000/day on all endpoints | Confirmed by operator |
