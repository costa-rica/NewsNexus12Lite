...
# NewsNexus12Lite — Pipeline TODO V02
...
- [ ] Create `api/src/types/pipeline.ts` (or extend `api/src/types/index.ts`):
  ...
  - Define `PipelineRun` interface:
    ...
    interface PipelineRun {
      runId: string;
      sessionId: string;
      articleIds: string[]; // ordered list selected for this run; max 10
      status: RunStatus;
...
- [ ] Create `api/src/services/runStore.ts` (if not already created in Backend TODO Phase 7):
  ...
  - `createRun(sessionId: string, runId: string, articleIds: string[]): PipelineRun` — initializes a `PipelineRun` with `sessionId`, ordered `articleIds`, all `stageStatuses` set to `"pending"`, all `articleProgress` initialized, `cancellationRequested: false`.
...
- [ ] Create `api/src/services/pipelineRunner.ts`:
  ...
  - `run.articleIds` is the single source of truth for ordered article iteration and max-10 defense-in-depth checks; do not derive article IDs from all `session.articles` or from object key order.
...
- [ ] Implement the per-stage execution loop inside `runPipeline` for each stage:
  ...
  4. Iterate through `run.articleIds` in order. For each article:
...
- [ ] Create `api/src/services/stages/aiApprover.ts`:
  ...
  - Executes in two sub-phases:
    ...
    2. **Hazard phase** (only if gateway is relevant): launch all three hazard prompts concurrently and preserve partial results if one fails. Use `Promise.allSettled` or wrap each hazard promise so every branch resolves to `{ hazardType, status: "complete" | "failed", result?, error? }`:
       ...
       - Do not use plain `Promise.all` without per-promise error wrapping because it rejects on the first failure and can lose successful hazard outputs.
...
- [ ] Write tests for each stage implementation in `api/src/services/stages/__tests__/`:
  ...
  - AI Approver gateway relevant path launches all three hazard prompts concurrently.
  - AI Approver normalization correctly assigns each of the four `finalStatus` values.
  - A failed hazard call after a relevant gateway preserves successful hazard results and sets `finalStatus: "needs_review"` or `"failed"`.
...
- [ ] In `pipelineRunner.ts`, add a guard at the top of `runPipeline`:
  - If `run.articleIds.length > 10`: throw an error (or log and set `run.status = "failed"` with an error message). This is a defense-in-depth check since the route should have already rejected >10 articles with HTTP 400.
...
- [ ] Write tests confirming the cap:
  ...
  - `runPipeline` guard: if called with `run.articleIds.length > 10` (should never happen), it sets run status to failed.
...
- [ ] Write tests for cancellation boundary behavior:
  ...
  - Cancellation requested before Stage 2 begins: pipeline exits at the pre-Stage-2 boundary; articles retain Stage 1 results and Stage 2 remains `"pending"`.
  - Cancellation requested during Stage 2 processing: pipeline finishes Stage 2 for all articles, then exits at the post-Stage-2 boundary; articles retain Stage 2 results and Stage 3 remains `"pending"`.
...
- [ ] Implement pipeline error handling per Pipeline Plan V02 §10:
  ...
  - [ ] **One hazard call failure after a relevant gateway**: use the `Promise.allSettled`/wrapped-promise hazard result shape from Phase 3; store partial hazard results for the calls that succeeded, store a failed hazard entry for the call that failed, set `finalStatus` to `"needs_review"` or `"failed"` according to the normalization rules, and log the specific hazard failure.
