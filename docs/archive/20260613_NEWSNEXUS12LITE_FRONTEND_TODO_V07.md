...
# NewsNexus12Lite — Frontend TODO V07
...
- [ ] Create `portal/src/store/orchestrationSlice.ts`:
  ...
  - Thunk: `startRun` — calls `POST /api/orchestration/runs` with current article IDs; on success sets `runId` and dispatches `startSnapshotPolling`. Do **not** set `isPolling: true` in `startRun`; the polling thunk owns that flag after its duplicate-loop guard passes.
...
- [ ] Create `portal/src/lib/apiClient.ts`:
  ...
  - On HTTP 429: extract `Retry-After` header and throw a typed `RateLimitError` (or equivalent typed error object) that includes `retryAfter`. Keep `apiClient.ts` side-effect-light: it must not import the Redux store or dispatch `uiSlice.addToast` directly.
...
- [ ] Create `portal/src/app/layout.tsx` as the root App Router **server** layout:
  - Do **not** mark `layout.tsx` with `"use client"`. Keep it responsible for `<html>`, `<body>`, metadata/title, and static shell composition only.
  - Set metadata/title to "NewsNexus Lite" using the App Router metadata API or equivalent supported Next.js 16 pattern.
  - Render a dedicated client component such as `<ClientShell>{children}</ClientShell>` inside `<body>`.
- [ ] Create `portal/src/components/ClientShell.tsx` (or `portal/src/app/providers.tsx`) with `"use client"`:
  ...
  - Render `<FirstLaunchModal>` at the client shell level so it overlays all pages.
  - Render a global toast/banner area (`<ToastArea>`).
  - Own all client-only hooks/effects needed by the shell, including session initialization in Phase 4.
...
- [ ] Write component tests for `<ClientShell>` and `<RightSidebar>`:
  - `<ClientShell>` renders the Redux/Persist provider wrapper, right sidebar, first-launch modal mount point, toast area, and children.
  - `<ClientShell>` dispatches `initSession` on mount from the client component, not from `app/layout.tsx`.
...
- [ ] In the client shell component (`ClientShell.tsx` or `providers.tsx`), add a `useEffect` that dispatches `initSession` on mount:
  - Do **not** put this `useEffect` in `app/layout.tsx`; the root layout remains a server component.
...
- [ ] Create `portal/src/app/page.tsx` as the integrated `/` pipeline page:
  - Use Next.js App Router file-based routing for the root route `/`.
  - Compose the main pipeline UI by rendering `<RssSearchForm>`, `<ArticleTable>`, and `<OrchestrationPanel>` together in the page content area provided by the root layout/client shell.
  - Arrange the article table and orchestration panel so the panel remains alongside the table as described above, while the global right sidebar continues to come from `<ClientShell>` / `<RightSidebar>` rather than being duplicated in the page.
  - If the root page needs client-only Redux hooks or effects, keep `page.tsx` as a server wrapper and render a dedicated client component (for example `<PipelinePage>`) that owns those hooks; otherwise `page.tsx` may directly compose client components without adding route-level effects.
- [ ] Write an integration/component test for the `/` pipeline page:
  - Rendering the page includes `<RssSearchForm>`, `<ArticleTable>`, and `<OrchestrationPanel>`.
  - The test should not require the right sidebar to be rendered by `page.tsx`; sidebar coverage remains in the root layout/client shell tests.
...
- [ ] Start the dev server with mock backend. Open `/` and confirm it is the main integrated pipeline page:
  - The page renders `<RssSearchForm>`, `<ArticleTable>`, and `<OrchestrationPanel>` together.
  - The global right sidebar is present from the root shell and is not duplicated inside `page.tsx`.
  - Start a run and confirm stage indicators animate through each stage.
...
- [ ] Implement the `startSnapshotPolling` thunk in `orchestrationSlice.ts`:
  ...
  - Duplicate poll guard: check `orchestration.isPolling` before starting a new loop. If already polling for the current run, do not start another loop. If not already polling, dispatch `orchestrationSlice.setPolling(true)` only after this guard passes and before creating the timer/recursive timeout loop.
...
- [ ] Write tests for the polling logic:
  - Polling starts when `startRun` succeeds: `startRun` stores `runId`, `startSnapshotPolling` passes the duplicate guard, then `startSnapshotPolling` sets `isPolling: true` and creates the loop.
...
- [ ] Create `portal/src/app/prompts/approver/page.tsx`:
  ...
  - Keep this `page.tsx` as a server route wrapper unless there is a deliberate reason to mark the route itself with `"use client"`.
  - Do **not** put `useEffect`, Redux hooks, or `loadPrompts` dispatch logic directly in a server `page.tsx`.
  - Render the dedicated client component `<ApproverPromptsPage />`, which owns prompt loading and page interactivity.
- [ ] Create `portal/src/components/ApproverPromptsPage.tsx` (the page's primary client component):
  - Mark the component file with `"use client"`.
  - On mount inside this client component: dispatches `promptsSlice.loadPrompts` thunk if `prompts.session` is null.
...
- [ ] Write tests for `<ApproverPromptsPage>`:
  - Server `app/prompts/approver/page.tsx` renders the client `<ApproverPromptsPage />` wrapper without owning mount effects.
  - Client `<ApproverPromptsPage>` dispatches `loadPrompts` on mount only from the client component when prompts are not already loaded.
...
- [ ] Create `portal/src/app/prompts/state-assigner/page.tsx`:
  ...
  - Keep this `page.tsx` as a server route wrapper unless there is a deliberate reason to mark the route itself with `"use client"`.
  - Do **not** put `useEffect`, Redux hooks, or `loadPrompts` dispatch logic directly in a server `page.tsx`.
  - Render the dedicated client component `<StateAssignerPromptsPage />`, which owns prompt loading and page interactivity.
- [ ] Create `portal/src/components/StateAssignerPromptsPage.tsx`:
  - Mark the component file with `"use client"`.
  - On mount inside this client component: dispatches `promptsSlice.loadPrompts` if prompts are not yet loaded.
...
- [ ] Write tests for `<StateAssignerPromptsPage>`:
  - Server `app/prompts/state-assigner/page.tsx` renders the client `<StateAssignerPromptsPage />` wrapper without owning mount effects.
  - Client `<StateAssignerPromptsPage>` dispatches `loadPrompts` on mount only from the client component when prompts are not already loaded.
...
- [ ] Implement global 429 rate-limit handling without creating an API-client/store import cycle:
  - In `apiClient.ts`, when any API call returns HTTP 429, extract the `Retry-After` header value and throw the typed `RateLimitError` described in Phase 2.
  - In Redux thunks and/or a shared thunk error helper, catch `RateLimitError` and dispatch `uiSlice.addToast` with message: **"Too many requests — please wait before trying again."** Include the retry-after duration if available.
  - Do **not** have `apiClient.ts` import the Redux store or dispatch `uiSlice.addToast` directly.
...
- [ ] Implement the reset-during-active-run flow (EC-011):
  - If the user clicks "Reset Demo" while a run is active: first call `cancelRun`, then clear state. Discard any late snapshot responses from the old run via the Phase 9 polling-layer run-ID guard in `startSnapshotPolling`, which compares top-level `snapshot.run.runId` to the current `orchestration.runId` before dispatching article updates.
...
- [ ] Write integration-style tests for error flows:
  - API client throws a typed `RateLimitError` with `retryAfter` on HTTP 429.
  - Thunk/shared error handler catches `RateLimitError` and dispatches the correct toast.
